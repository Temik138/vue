<template>
  <AdminLayout>
    <div class="admin-form-container">
      <h1 class="admin-title">Редактировать товар: {{ product.name }}</h1>
      ...
      <form @submit.prevent="submit" class="product-form">
        ...
        <div class="form-group">
          ...
          >
          <p v-if="product.image" class="current-image-label">Текущее изображение:</p>
          <img v-if="product.image" :src="`/storage/${product.image}`" alt="Текущее изображение товара" class="current-product-image">
        ...
        >
          {{ form.processing ? 'Обновление...' : 'Обновить товар' }}
        </button>
      </form>
    </div>
  </AdminLayout>
...
const form = useForm({
  _method: 'put', // Важно: для PUT-запросов Inertia.js требует _method: 'put'
  name: props.product.name,
  description: props.product.description,
  price: props.product.price,
  category: props.product.category,
  image: null, // Для нового изображения
});
